...
def parse_sci_tec(response, category_name):
    ...
    reviews_gadgets_featured_games = response.xpath('//div[@class="technology-featured-section '
                                                    'technology-featured-section2"]')
    ...
    for entry in entries:
        ...
        image_urls = entry.xpath('.//div/a/div/img/@src').extract()
...
def parse_pakistan(response, category_name):
    ...
    GB_AJK_KP_B_S_P_I = response.xpath('//div[(@class="col-md-12")]')
    more_news = response.xpath('//div[(@class="sports-demo")]')

    entries = top_top + top + GB_AJK_KP_B_S_P_I + more_news
    news_items = []
            ...
            for url in urls:
                if url and content:
                    ...
                    news_item = News(
                        ...
                        url=url,
                        image_url=img,
                        news_image="https://th.bing.com/th/id/R.016d2930c1bb05504385260ff6a6a9bf?rik=IdtbqSCn%2fvmH0g"
                                   "&pid=ImgRaw&r=0",
                        category=category
                    )
                    news_items.append(news_item)
...
def parse_Business(response, category_name):
    ...
    L_M_N = response.xpath('//div[(@class="latest-market-news")]')
    L_M_N2 = response.xpath('//div[(@class="latest-market-news-small-thumbs")]')
    ...
    entries = top + L_M_N + L_M_N2 + featured + more_news + more_news2
    news_items = []
...
def parse_sports(response, category_name):
    top_top = response.xpath('//div[(@class="sport-section1")]')
    top = response.xpath('//div[(@class="horizontal-news1 sportsecton2")]')
    featured = response.xpath('//div[(@class="sports-features-wrapper")]')
    cricket_football_hockey = response.xpath('//div[(@class="sport-morestores-inner")]')
    more_news = response.xpath('//div[(@class="sports-demo")]')

    entries = top_top + top + featured + cricket_football_hockey + more_news
    news_items = []
